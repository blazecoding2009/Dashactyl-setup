...
sed 's/4440/$port/1' settings.yml
sed 's/Enter your session password here./$cookiepassword/1' settings.yml
...
fullurl = secure + url
sed 's/http://localhost:8000/$fullurl/1' settings.yml
sed 's/localhost:4440/$url/1' settings.yml


read -p "Enter the panel url include http:// or https://): " panelurl
read -p "Enter the admin API key of panel: " panelapi

sed 's/Enter your Pterodactyl Panel domain here./$panelurl/1' settings.yml
sed 's/Enter your Pterodactyl Panel application API key here./$panelapi/1' settings.yml
...
if [[ webhook_prompt == "y"]]; then
      ...
      sed 's/false # enable or disable the Audit Logs/true # enable or disable the Audit Logs/1' settings.yml
      sed 's/Enter your webhook URL/$webhook_url/1' settings.yml
fi
